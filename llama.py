...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        if exec_type == "prefill":
            ...
            if batch_id_mask is None:
                y = self.decoding(hidden_states, position_embeddings, cache_lens)
            else:
                y = self.mask_decoding(hidden_states, position_embeddings, cache_lens, batch_id_mask)
    ...
    def tree_decoding(
        ...
        k_cache = self.K_Cache[:, :cache_lens].transpose(1, 2)
        v_cache = self.V_Cache[:, :cache_lens].transpose(1, 2)

        self.K_Cache[:, cache_lens: cache_lens + q_len] = key_states.transpose(1, 2)
        self.V_Cache[:, cache_lens: cache_lens + q_len] = value_states.transpose(1, 2)
        ...
        attn_weights = attn_weights + tree_mask
    ...
    def mask_decoding(self, hidden_states, position_embeddings, cache_lens, batch_id_mask):
        # assert hidden_states.size(1) == 1, f"Only support decoding with one token at the dim 1 at a time, current size is {hidden_states.size()}"
        bsz, q_len = batch_id_mask.size()
        new_hidden = hidden_states.new_zeros((bsz, q_len, hidden_states.size(-1)))
        new_hidden[batch_id_mask] = hidden_states

        query_states = self.q_proj(new_hidden)
        key_states = self.k_proj(new_hidden)
        value_states = self.v_proj(new_hidden)
        ...
        attn_output = attn_output[batch_id_mask]
        attn_output = self.o_proj(attn_output)
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if flag:
            for i in range(bsz):
                hidden_states[i] = self.input_layernorm(hidden_states[i])
        else:
        ...
        if flag:
            for i in range(bsz):
                hidden_states[i] = self.post_attention_layernorm(hidden_states[i])
        ...
        if position_ids is None:
            if tree_mask is None:
                if batch_id_mask is None:
                    ...
                    bsz, q_len = batch_id_mask.size()
                    position_ids = torch.arange(0, q_len)[None, :].to(input_ids.device)
            else:
...
class LlamaForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
